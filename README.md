...
- **Custom exceptions**: 4 unchecked exceptions for distinct game scenarios (duplicate entity, entity not found, entity already dead, invalid data)
...
- Some starting mob stats are hardcoded rather than configurable
- Duplicate sorting logic in `EntityManager` could be refactored into a single generic method accepting a `Comparator`